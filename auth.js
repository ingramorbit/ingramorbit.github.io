...
function applyAdminBtn(user) {
  ...
  if (!adminBtn) return;
  const isAdmin = user && (user.tier === "admin" || ADMIN_EMAILS.includes(user.email));
...
function applyTierUI(user) {
  applyAdminBtn(user);
  if (!plansBtn) return;
  if (!user) { plansBtn.textContent = "View Plans"; plansBtn.href = "plans.html"; return; }
  if (user.tier === "free")  { plansBtn.textContent = "Upgrade to Gold"; plansBtn.href = "plans.html"; return; }
  if (user.tier === "gold")  { plansBtn.textContent = "Gold Member"; plansBtn.href = "plans.html"; return; }
  if (user.tier === "admin") { plansBtn.textContent = "Admin Panel"; plansBtn.href = "all-requests.html"; return; }
...
async function loadUserDoc(firebaseUser) {
  ...
  const isOwner = ADMIN_EMAILS.includes(firebaseUser.email);
  try {
    ...
    if (userSnap.exists()) {
      ...
      return {
        ...
        tier: isOwner ? "admin" : (data.tier || "free"),
    ...
    return {
      ...
      email: firebaseUser.email || "",
      displayName: firebaseUser.displayName || "",
      name: firebaseUser.displayName || (isOwner ? "John" : ""),
